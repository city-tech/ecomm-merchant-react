...
const Checkout = () => {
    ...
    const initializeGetPay = () => {
        ...
        try {
            const getPayInstance = new window.GetPay(options);

            // some SDKs use initialize, some use init
            if (typeof getPayInstance.initialize === "function") {
                getPayInstance.initialize();
            } else if (typeof getPayInstance.init === "function") {
                getPayInstance.init();
            } else {
                console.error("No valid init method found on GetPay");
                setIsLoading(false);
            }
        } catch (err) {
